
<!DOCTYPE html>
<head>
    <title>Tomorrow X Together</title>
    <link rel="stylesheet" href="../styles/navigation.css">
    <link rel="stylesheet" href="../styles/song.css">
    <link rel="stylesheet" href="../styles/button.css" >
    <link rel="stylesheet" href="https://unicons.iconscout.com/release/v4.0.0/css/line.css" />
</head>

<body>
    <section class="section-home">
    <div class="song-background">
        <?php include("../includes/navigation.php") ?>
        <?php include("../includes/search.php") ?>
        <table border="0" class="song-table">
            <tbody>
                <tr>
                    <td>
                        <img src="../images/yeonjun2.jpg" width="300" height="440" >
                    </td>
                    <td>
                    <h2>Yeonjun</h2>
                    <br>Stage Name: Yeonjun (연준)<br>
                    Birth Name: Choi Yeon Jun (최연준)<br>
                    English Name: Daniel Choi<br>
                    Position: -<br>
                    Birthday: September 13th, 1999<br>
                    Zodiac Sign: Virgo<br>
                    Chinese Zodiac: Hare/Rabbit<br>
                    Height: 181.5 cm (5’11”)<br>
                    Weight: 62 kg (136 lbs)<br>
                    Blood Type: A<br>
                    MBTI Type: ENFP<br>
                    Representative Emoticon: 🦊<br>
                    Nationality: Korean<br>
                    Spotify Playlist: <a href="https://open.spotify.com/playlist/5IeEI4NWGhDyFRi9aZHVOZ?si=YUw9obYWT1SUUUlrD4ET1Q" target="_blank" rel="nofollow">TXT: YEONJUN</a><br>
                    Instagram: <a href="https://www.instagram.com/yawnzzn/" target="_blank" rel="nofollow">yawnzzn</a>
                    </td>
                    <td>
                        <img src="../images/yeonjun3.jpg" width="300" height="440" >
                    </td>
                </tr>
            </tbody>
        </table>
        <div class="container">
            <a href="../profile/yeonjun.php"><span>Yeonjun</span></a>
            <a href="../profile/taehyun.php"><span>Taehyun</span></a>
            <a href="../profile/beomgyu.php"><span>Beomgyu</span></a>
            <a href="../profile/hueningkai.php"><span>Huening Kai</span></a>
            <a href="../profile/soobin.php"><span>Soobin</span></a>
        </div>
</div>
</body>
</html>
